...
class OrderHelper {
	...
	public function alma_do_checkout_in_page() {
		...
		if (
			...
			$this->return_error( __( 'Invalid data', 'alma-gateway-for-woocommerce' ) );
		}
	...
	private function return_error( $message ) {
		wc_add_notice( $message, ConstantsHelper::ERROR );
	...
	protected function create_inpage_order( $post_fields ) {
		...
		if ( ! empty( $post_fields['fields'] ) && ! empty( $post_fields['fields']['extensionData'] ) ) {
			$params = $post_fields['fields']['extensionData']['woocommerce/order-attribution'];
			if ( ! empty( $params ) ) {
				do_action( 'woocommerce_order_save_attribution_data', $order, $params ); // phpcs:ignore
			}
		}
