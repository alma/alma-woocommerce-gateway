...
class Alma_WC_Shortcodes {
	...
	public function init_cart_widget_shortcode( Alma_WC_Cart_Handler $handler ) {
		add_shortcode(
			Alma_WC_Helper_Constants::CART_SHORTCODE_TAG,
	 ...
	 */
	protected function alma_cart_widget( Alma_WC_Cart_Handler $handler, $atts, $content = '' ) {

		if ( $handler->is_already_rendered() ) {

			return $this->render_empty( Alma_WC_Helper_Constants::CART_SHORTCODE_TAG, $handler->get_eligibility_widget_already_rendered_message(), $atts, $content );
		}

		ob_start();
		$handler->display_cart_eligibility();

		return $this->render( Alma_WC_Helper_Constants::CART_SHORTCODE_TAG, $atts, ob_get_clean(), $content );
	}
	...
	public function init_product_widget_shortcode( Alma_WC_Product_Handler $handler ) {
		add_shortcode(
			Alma_WC_Helper_Constants::PRODUCT_SHORTCODE_TAG,
			function ( $atts, $content = '' ) use ( $handler ) {
	 ...
	 */
	protected function alma_product_widget( Alma_WC_Product_Handler $handler, $atts, $content = '' ) {

		if ( $handler->is_already_rendered() ) {
			return $this->render_empty( Alma_WC_Helper_Constants::PRODUCT_SHORTCODE_TAG, $handler->get_eligibility_widget_already_rendered_message(), $atts, $content );
		}
		...
		if ( ! $product ) {
			...
			return $this->render_empty( Alma_WC_Helper_Constants::PRODUCT_SHORTCODE_TAG, $product_not_found_content, $atts, $content );
		}

		ob_start();
		$handler->inject_payment_plan( $product );

		return $this->render( Alma_WC_Helper_Constants::PRODUCT_SHORTCODE_TAG, $atts, ob_get_clean(), $content );
	}
