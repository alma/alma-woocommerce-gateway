	...
	private function init_technical_fields( $default_settings ) {
		...
		return array(
			...
			'variable_product_check_variations_event' => array(
				...
				// translators: %$1s is a technical information, %$2s is the Alma FAQ URL.
				'description' => sprintf( __( 'This is the javascript event triggered to refresh the widget on a variable product page when the customer changes the product variation. Default value is <strong>$%1s</strong>.<br />More technical informations on <a href="$%2s" target="_blank">Alma documentation</a>.', 'alma-woocommerce-gateway' ), Alma_WC_Settings::DEFAULT_CHECK_VARIATIONS_EVENT, 'https://docs.getalma.eu/docs/woocommerce-faq' ),
			),
