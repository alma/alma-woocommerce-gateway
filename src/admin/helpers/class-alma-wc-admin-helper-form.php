 ...
 * Alma_WC_Admin_Helper_Form.
 */
class Alma_WC_Admin_Helper_Form {
	 ...
	 *
	 * @var Alma_WC_Payment_Upon_Trigger
	 */
	...
	public function __construct() {
		$this->settings_helper      = new Alma_WC_Settings();
		$this->payment_upon_trigger = new Alma_WC_Payment_Upon_Trigger();
	}
	...
	public function init_form_fields( $show_alma_fee_plans ) {
		...
		$default_settings = Alma_WC_Helper_Settings::default_settings();
	...
	public function init_api_key_fields( $keys_title, $default_settings ) {
		...
		return array(
			'keys_section' => array(
				...
				'description' => sprintf( __( 'You can find your API keys on <a href="%s" target="_blank">your Alma dashboard</a>', 'alma-gateway-for-woocommerce' ), Alma_WC_Helper_Assets::get_alma_dashboard_url( $this->settings_helper->get_environment(), 'security' ) ),
			),
			'live_api_key' => array(
	...
	public function init_debug_fields( $default_settings ) {
		return array(
			...
			'debug'         => array(
				...
				'label'       => __( 'Activate debug mode', 'alma-gateway-for-woocommerce' ) . sprintf( __( '(<a href="%s">Go to logs</a>)', 'alma-gateway-for-woocommerce' ), Alma_WC_Helper_Assets::get_admin_logs_url() ),
	...
	protected function init_fee_plans_fields( $default_settings ) {
		...
		if ( count( $select_options ) === 0 ) {
			...
			$title_field['fee_plan_section']['description'] = sprintf( __( '⚠ There is no fee plan allowed in your <a href="%s" target="_blank">Alma dashboard</a>.', 'alma-gateway-for-woocommerce' ), Alma_WC_Helper_Assets::get_alma_dashboard_url( $this->settings_helper->get_environment(), 'conditions' ) );
		...
		return array_merge(
			...
			array(
				'selected_fee_plan' => array(
					...
					/* translators: %s: Alma conditions URL */
					'description' => sprintf( __( 'Choose which fee plan you want to modify<br>(only your <a href="%s" target="_blank">Alma dashboard</a> available fee plans are shown here).', 'alma-gateway-for-woocommerce' ), Alma_WC_Helper_Assets::get_alma_dashboard_url( $this->settings_helper->get_environment(), 'conditions' ) ),
	...
	protected function init_fee_plan_fields( FeePlan $fee_plan, $default_settings, $selected ) {
		...
		$default_min_amount    = Alma_WC_Helper_Tools::alma_wc_price_from_cents( $fee_plan->min_purchase_amount );
		$default_max_amount    = Alma_WC_Helper_Tools::alma_wc_price_from_cents( $fee_plan->max_purchase_amount );
		$merchant_fee_fixed    = Alma_WC_Helper_Tools::alma_wc_price_from_cents( $fee_plan->merchant_fee_fixed );
		$merchant_fee_variable = $fee_plan->merchant_fee_variable / 100; // percent.
		$customer_fee_fixed    = Alma_WC_Helper_Tools::alma_wc_price_from_cents( $fee_plan->customer_fee_fixed );
	...
	protected function generate_i18n_field( $field_name, $field_infos, $default ) {
		$lang_list = Alma_WC_Helper_Internationalization::get_list_languages();

		if (
			Alma_WC_Helper_Internationalization::is_site_multilingual()
			&& count( $lang_list ) > 0
		) {
			$new_fields = array();
			...
			foreach ( $lang_list as $code_lang => $label_lang ) {
				...
				$new_field_infos['default']   = Alma_WC_Helper_Internationalization::get_translated_text( $default, $code_lang );
		...
		return array_merge(
			...
			array(
				...
				'payment_upon_trigger_display_text' => array(
					...
					'options'     => Alma_WC_Helper_General::get_display_texts_keys_and_values(),
				),
		...
		return array(
			...
			'variable_product_check_variations_event'    => array(
				...
				// translators: %1$s is technical information, %2$s is Alma WooCommerce Plugin FAQ doc URL.
					...
					Alma_WC_Helper_Constants::DEFAULT_CHECK_VARIATIONS_EVENT,
	 ...
	 * @return float
	 */
	public function almapay_wc_price_from_cents( $price ) {
