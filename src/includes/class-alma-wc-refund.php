...
class Alma_WC_Refund {
	...
	public function woocommerce_order_fully_refunded( $order_id, $refund_id ) {
		...
		/* translators: %s is a username. */
//		$this->refund_helper->add_order_note( $order_id, 'success', sprintf( __( 'Order fully refunded via Alma by %s.', 'alma-woocommerce-gateway' ), wp_get_current_user()->display_name ) );
