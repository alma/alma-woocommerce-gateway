...
class Alma_WC_Payment_Validator {
	...
	public static function validate_payment( $payment_id ) {
		...
		try {
			...
			$logger->error( 'Error while fetching payment with id.', [
                'Method' => __METHOD__ ,
		...
		try {
            ...
            $logger->error( 'Error getting order associated to payment.', [
                'Method' => __METHOD__ ,
                'PaymentId' => $payment_id,
                'ExceptionMessage' => $e->getMessage()
            ]);
		...
		if ( $order->get_wc_order()->has_status( apply_filters( 'alma_wc_valid_order_statuses_for_payment_complete', array( 'on-hold', 'pending', 'failed', 'cancelled' ) ) ) ) {
			if ( $order->get_total() !== $payment->purchase_amount ) {
				$logger->error( 'The total order amount does not match the purchase amount of the payment.', [
                    'Method' => __METHOD__,
				...
				try {
					...
					$logger->error( 'FlagAsPotentialFraud request error (total amount issue).', [
                        'Method' => __METHOD__,
                        'ExceptionMessage'  => $e->getMessage()
                    ]);
				}
			...
			if ( ! in_array( $payment->state, array( Payment::STATE_IN_PROGRESS, Payment::STATE_PAID ), true ) ||
				...
				$logger->error('State payment error/incorrect.', [
                    'Method' => __METHOD__,
                    'OrderId' => $order->get_id(),
                    'PaymentId' => $payment_id,
				...
				try {
                    ...
                    $logger->error( 'FlagAsPotentialFraud request error (state payment incorrect).', [
                        'Method' => __METHOD__,
                        'ExceptionMessage'  => $e->getMessage()
                   ]);
				}
