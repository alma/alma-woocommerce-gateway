...
class Alma_Check_Legal_Helper {
	...
	public function init() {
		...
		wp_enqueue_style( 'alma-admin-styles-modal-checkout-legal', Alma_Assets_Helper::get_asset_admin_url( 'css/alma-modal-checkout-legal.css' ), array(), ALMA_VERSION );

		wp_enqueue_script(
			'alma-admin-scripts-modal-checkout-legal',
			Alma_Assets_Helper::get_asset_admin_url( 'js/alma-modal-checkout-legal.js' ),
			array( 'jquery' ),
			ALMA_VERSION,
			true
		);
	...
	public function get_modal_checkout_legal() {
		...
		$alma_logo = Alma_Assets_Helper::get_icon( $this->get_modal_title(), self::ID );
