...
class Alma_WC_Payment_Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		$this->id                 = Alma_WC_Helper_Constants::GATEWAY_ID;
		...
		$this->logger->error( 'init_gateway' );
		$this->alma_settings   = new Alma_WC_Settings();
		$this->checkout_helper = new Alma_WC_Helper_Checkout();
		$this->gateway_helper  = new Alma_WC_Helper_Gateway();
		$this->general_helper  = new Alma_WC_Helper_General();
		$this->scripts_helper  = new Alma_WC_Helper_Assets();
		$this->plan_builder    = new Alma_WC_Plan_Builder();
	...
	public function check_activation() {
		...
		if ( ! Alma_WC_Helper_Tools::alma_wc_string_to_bool( $enabled ) ) {
			$message = sprintf(
				...
				esc_url( Alma_WC_Helper_Assets::get_admin_setting_url( false ) )
			);
			alma_wc_plugin()->admin_notices->add_admin_notice( 'no_alma_enabled', 'notice notice-warning', $message, true );
		}
	}
	...
	public function get_option( $key, $empty_value = null ) {
		...
		if ( Alma_WC_Helper_Tools::is_amount_plan_key( $key ) ) {
			return strval( Alma_WC_Helper_Tools::alma_wc_price_from_cents( $option ) );
		}
	...
	public function init_admin_form() {
		...
		if (
				...
				&& Alma_WC_Helper_Constants::GATEWAY_ID == $current_section
		) {
			...
			$this->form_admin = new Alma_WC_Admin_Helper_Form();
	 ...
	 *
	 * @return void
	 * @throws Alma_WC_Exception_Api_Client Client Api exception.
	 * @throws Alma_WC_Exception_Api_Merchants Merchant Api exception.
	 * @throws Alma_WC_Exception_Api_Plans  Plans Api exception.
	 * @throws Alma_WC_Exception_No_Credentials No credentials Api excetption.
	 */
	public function manage_credentials() {
		...
		$this->alma_settings->manage_allowed_fee_plans();

		// Save of the fee plans.
		update_option( $this->get_option_key(), apply_filters( 'woocommerce_settings_api_sanitized_fields_' . $this->id, $this->alma_settings->settings ), 'yes' );
	}
	 ...
	 *
	 * @return void
	 * @throws Alma_WC_Exception_No_Credentials No credentials.
	 */
	protected function check_alma_keys() {
		...
		if ( ! $this->alma_settings->has_keys() ) { // nope.
			...
			// translators: %s: Admin settings url.
				...
				esc_url( Alma_WC_Helper_Assets::get_alma_dashboard_url( $this->alma_settings->get_environment(), 'security' ) )
			);

			alma_wc_plugin()->admin_notices->add_admin_notice( 'no_alma_keys', 'notice notice-warning', $message, true );

			throw new Alma_WC_Exception_No_Credentials( $message );
		}
	}
	 ...
	 *
	 * @return void
	 * @throws Alma_WC_Exception_Api_Client Issue with the api.
	 */
	protected function get_alma_client() {
		try {
			...
			// translators: %1$s: Admin settings url, %2$s: Admin logs url.
				...
				esc_url( Alma_WC_Helper_Assets::get_admin_logs_url() )
			);

			alma_wc_plugin()->admin_notices->add_admin_notice( 'client_keys_build', 'notice notice-error', $message, true );

			throw new Alma_WC_Exception_Api_Client( $message );
		}
	}
	 ...
	 *
	 * @return void
	 * @throws Alma_WC_Exception_Api_Merchants Api error.
	 */
	protected function get_alma_merchant() {
		try {
			// We try to get the merchants.
			$this->alma_settings->get_alma_merchant_id();
		} catch ( Exception $e ) {
			alma_wc_plugin()->admin_notices->add_admin_notice( 'error_keys', 'notice notice-error', $e->getMessage(), true );
			throw new Alma_WC_Exception_Api_Merchants( $e );
		}
	...
	public function init_form_fields() {
		$this->form_fields = $this->form_admin->init_form_fields( $this->show_alma_fee_plans );
		$this->form_fields = apply_filters( 'wc_offline_form_fields', $this->form_fields );
	}
	...
	public function get_icon() {
		$icon_url = Alma_WC_Helper_Assets::get_asset_url( 'images/alma_logo.svg' );
	...
	public function is_available() {
		$tools = new Alma_WC_Helper_Tools();

		if ( ! $this->alma_settings->is_allowed_to_see_alma( wp_get_current_user() ) ) {
			return false;
		}
		...
		return $this->gateway_helper->is_cart_eligible() && parent::is_available();
	}
		...
		foreach ( $this->get_form_fields() as $key => $field ) {
			if ( Alma_WC_Helper_Tools::is_amount_plan_key( $key ) ) {
				try {
					...
					$this->alma_settings->settings[ $key ] = Alma_WC_Helper_Tools::alma_wc_price_to_cents( $amount );
				} catch ( Exception $e ) {
					$this->add_error( $e->getMessage() );
				}
			}
	...
	public function payment_fields() {
		...
		<p>
			<?php
			$this->plan_builder->render_payment_plan( $gateway_id, $default_plan );
			?>
		</p>
		<?php
	}
