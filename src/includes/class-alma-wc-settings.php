...
class Alma_WC_Settings {
	...
	public static function get_settings() {
		...
		return array_merge( Alma_WC_Helper_Settings::default_settings(), $settings );
	}
	...
	public function get_i18n( $key ) {
		if ( Alma_WC_Helper_Internationalization::is_site_multilingual() ) {
			...
			return Alma_WC_Helper_Internationalization::get_translated_text(
				Alma_WC_Helper_Settings::default_settings()[ $key ],
				get_locale()
			);
		}
	 ...
	 *
	 * @return Payment
	 * @throws Alma_WC_Exception_Api_Create_Payments Create payment exception.
	 */
	public function create_payments( $order_id, $fee_plan_definition ) {
		try {
			$model_payment = new Alma_WC_Model_Payment();
			...
			throw new Alma_WC_Exception_Api_Create_Payments( $order_id, $fee_plan_definition );
		}
	}
	 ...
	 * @throws Alma_WC_Exception_Alma General exception.
	 */
	public function get_alma_client() {
		...
		$this->logger->debug( 'No alma client ' );

		throw new Alma_WC_Exception_Alma(
	 ...
	 * @return Payment
	 *
	 * @throws Alma_WC_Exception_Api_Fetch_Payments Fetch payment exception.
	 */
	public function fetch_payment( $payment_id ) {
		try {
			...
			throw new Alma_WC_Exception_Api_Fetch_Payments( $payment_id );
		}
	}
	 ...
	 * @return Payment
	 *
	 * @throws Alma_WC_Exception_Api_Trigger_Payments Api trigger exception.
	 */
	public function trigger_payment( $transaction_id ) {
		try {
			...
			throw new Alma_WC_Exception_Api_Trigger_Payments( $transaction_id );
		}
	}
	 ...
	 *
	 * @return void
	 * @throws Alma_WC_Exception_Api_Full_Refund APi refund exception.
	 */
	public function full_refund( $transaction_id, $merchant_reference, $comment ) {
		try {
			...
			throw new Alma_WC_Exception_Api_Full_Refund( $transaction_id, $merchant_reference );
		}
	}
	 ...
	 *
	 * @return void
	 * @throws Alma_WC_Exception_Api_Partial_Refund APi refund exception.
	 */
	public function partial_refund( $transaction_id, $amount, $merchant_reference, $comment ) {
		try {
			...
			throw new Alma_WC_Exception_Api_Partial_Refund( $transaction_id, $merchant_reference, $amount );
		}
	}
	 ...
	 * @throws Alma_WC_Exception_Alma Alma exception.
	 */
	 ...
	 *
	 * @return void
	 * @throws Alma_WC_Exception_Alma_Activation Alma_WC_Exception_Alma_Activation.
	 * @throws Alma_WC_Exception_Api_Merchants Alma_WC_Exception_Api_Merchants.
	 * @throws Alma_WC_Exception_Wrong_Credentials Alma_WC_Exception_Wrong_Credentials.
	 * @throws Alma_WC_Exception_Alma General exceptions.
	 */
	public function get_alma_merchant_id() {
		...
		if ( ! empty( $this->alma_client ) ) {
			try {
				...
				$this->logger->debug( 'found merchant ' . $this->{$this->environment . '_merchant_id'} );

			} catch ( Exception $e ) {
				if ( $e->response && 401 === $e->response->responseCode ) {
					throw new Alma_WC_Exception_Wrong_Credentials( $this->get_environment() );
				}

				throw new Alma_WC_Exception_Api_Merchants(
					sprintf(
					// translators: %s: Error message.
			...
			if ( ! $can_create_payment ) {
				throw new Alma_WC_Exception_Alma_Activation( $this->environment );
			}
		} else {
			throw new Alma_WC_Exception_Alma(
			// translators: %s: Error message.
	 ...
	 *
	 * @return void
	 * @throws Alma_WC_Exception_Api_Plans Alma_WC_Exception_Api_Plans.
	 */
	public function manage_allowed_fee_plans() {
		...
		if ( ! $fee_plans ) {
			...
			alma_wc_plugin()->admin_notices->add_admin_notice( 'error_get_fee', 'notice notice-error', $message, true );

			throw new Alma_WC_Exception_Api_Plans( $message );
		}
		...
		foreach ( $this->allowed_fee_plans as $fee_plan ) {
			...
			if ( ! isset( $this->settings[ $enabled_key ] ) ) {
				$this->settings[ $enabled_key ] = Alma_WC_Helper_Constants::DEFAULT_FEE_PLAN === $plan_key ? 'yes' : 'no';
			}
	...
	public function build_new_available_gateways( $gateway ) {
		...
		if ( $this->is_there_available_plan_for_this_gateway( Alma_WC_Helper_Constants::ALMA_GATEWAY_PAY_LATER ) ) {
			$tmp_gateway                                = clone $gateway;
			$tmp_gateway->id                            = Alma_WC_Helper_Constants::ALMA_GATEWAY_PAY_LATER;
			$new_available_gateways[ $tmp_gateway->id ] = $tmp_gateway;
		}

		if ( $this->is_there_available_plan_for_this_gateway( Alma_WC_Helper_Constants::ALMA_GATEWAY_PAY_MORE_THAN_FOUR ) ) {
			$tmp_gateway                                = clone $gateway;
			$tmp_gateway->id                            = Alma_WC_Helper_Constants::ALMA_GATEWAY_PAY_MORE_THAN_FOUR;
			$new_available_gateways[ $tmp_gateway->id ] = $tmp_gateway;
		}
	...
	public function get_eligible_plans_keys_for_cart() {
		...
		return array_filter(
			$this->get_eligible_plans_keys( ( new Alma_WC_Model_Cart() )->get_total_in_cents() ),
	...
	public function get_cart_eligibilities() {
		if ( ! $this->eligibilities ) {
			...
			try {
				$this->get_alma_client();
				$this->eligibilities = $this->alma_client->payments->eligibility( Alma_WC_Model_Payment::get_eligibility_payload_from_cart() );
	...
	public function should_display_plan( $plan_key, $gateway_id ) {
		switch ( $gateway_id ) {
			case Alma_WC_Helper_Constants::GATEWAY_ID:
				...
				break;
			case Alma_WC_Helper_Constants::ALMA_GATEWAY_PAY_LATER:
				$should_display = (
					$this->get_installments_count( $plan_key ) === 1
					&& ( $this->get_deferred_days( $plan_key ) !== 0 || $this->get_deferred_months( $plan_key ) !== 0 )
				);
				break;
			case Alma_WC_Helper_Constants::ALMA_GATEWAY_PAY_MORE_THAN_FOUR:
	...
	public function get_display_text() {
		return Alma_WC_Helper_General::get_display_texts_keys_and_values() [ $this->payment_upon_trigger_display_text ];
	}
	 ...
	 *
	 * @return array
	 * @throws Alma_WC_Exception_Plans_Definition Alma_WC_Exception_Plans_Definition.
	 */
	public function get_fee_plan_definition( $plan_key ) {
		...
		if ( ! isset( $this->settings[ "installments_count_$plan_key" ] ) ) {
			throw new Alma_WC_Exception_Plans_Definition( "installments_count_$plan_key not set" );
		}

		if ( ! isset( $this->settings[ "deferred_days_$plan_key" ] ) ) {
			throw new Alma_WC_Exception_Plans_Definition( "deferred_days_$plan_key not set" );
		}

		if ( ! isset( $this->settings[ "deferred_months_$plan_key" ] ) ) {
			throw new Alma_WC_Exception_Plans_Definition( "deferred_months_$plan_key not set" );
		}
