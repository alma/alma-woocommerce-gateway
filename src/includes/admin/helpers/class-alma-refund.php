...
class Alma_Refund {
	...
	public function get_display_refund_amount( $refund ) {
		return $refund->get_amount() . ' class-alma-refund.php' . $refund->get_currency();
	}
	...
	public function make_full_refund( $order, $refund_id = 0 ) {
		...
		if ( '1' === get_post_meta( $order->get_id(), Alma_Constants::FLAG_ORDER_FULLY_REFUNDED, true ) ) {
			return;
		}
		...
		try {
			...
			update_post_meta( $order->get_id(), Alma_Constants::FLAG_ORDER_FULLY_REFUNDED, '1' );
	...
	public function format_refund_comment( $comment ) {
		return Alma_Constants::PREFIX_REFUND_COMMENT . $comment;
	}
	...
	public function add_success_note( \WC_Order $order, $message ) {
		$this->add_order_note( $order, Alma_Constants::SUCCESS, $message );
	}
	...
	protected function add_notice( \WC_Order $order, $notice_type, $message ) {
		$refund_notices   = get_post_meta( $order->get_id(), Alma_Constants::REFUND_NOTICE_META_KEY );
		$refund_notices[] = array(
			'notice_type' => $notice_type,
			'message'     => $message,
		);
		update_post_meta( $order->get_id(), Alma_Constants::REFUND_NOTICE_META_KEY, $refund_notices );
	}
	...
	public function add_error_note( \WC_Order $order, $message ) {
		$this->add_order_note( $order, Alma_Constants::ERROR, $message );
	}
	...
	public function get_refund_amount( $refund ) {
		return Alma_Tools::alma_price_to_cents( floatval( $refund->get_amount() ) );
	}
	...
	public function is_paid_with_alma( $order ) {
		return in_array(
			...
			array(
				Alma_Constants::GATEWAY_ID,
				Alma_Constants::ALMA_GATEWAY_PAY_LATER,
				Alma_Constants::ALMA_GATEWAY_PAY_MORE_THAN_FOUR,
			),
			true
		);
	}
	...
	public function print_notices( \WC_Order $order ) {
		$refund_notices = get_post_meta( $order->get_id(), Alma_Constants::REFUND_NOTICE_META_KEY, true );
	...
	public function delete_notices( \WC_Order $order ) {
		delete_post_meta( $order->get_id(), Alma_Constants::REFUND_NOTICE_META_KEY );
	}
}
