	...
	protected function validate_checkout( &$data, &$errors ) {
		...
		if ( $this->cart_factory->get_cart()->needs_shipping() ) {
			...
			if ( empty( $shipping_country ) ) {
				...
				$errors->add( 'shipping', sprintf( __( 'Unfortunately <strong>we do not ship %s</strong>. Please enter an alternative shipping address.', 'alma-gateway-for-woocommerce' ), WC()->countries->shipping_to_prefix() . ' CheckoutService.php' . $this->customer_factory->get_shipping_country() ) );
			} else {
				$session = $this->session_factory->get_session();
