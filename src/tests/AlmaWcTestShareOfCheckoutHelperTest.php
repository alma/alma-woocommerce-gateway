...
class AlmaWcTestShareOfCheckoutHelperTest extends TestCase {
	...
	public function test_get_from_date( $from_date, $expected ) {
        $result = Alma_WC_Admin_Helper_Share_Of_Checkout::get_from_date($from_date);
	...
	public function test_get_to_date( $to_date, $expected ) {
        $result = Alma_WC_Admin_Helper_Share_Of_Checkout::get_to_date($to_date);
		$this->assertEquals( $expected, $result );
	}
	...
	public function test_get_payload( $orders_by_date_range_mock, $expectedPayload ) {
		$orderHelperMock       = Mockery::mock(Alma_WC_Admin_Helper_Order::class );
		$orderHelperMock->shouldReceive('get_orders_by_date_range' )->andReturn( $orders_by_date_range_mock );
		$random_date = '2022-01-01';
		$this->assertEquals( $expectedPayload, Alma_WC_Admin_Helper_Share_Of_Checkout::get_payload( $random_date ) );
	}
