...
class Almapay_WC_Shortcodes {
	 ...
	 *
	 * @param Almapay_WC_Cart_Handler $handler as cart widget handler.
	 *
	 * @return void
	 */
	public function init_cart_widget_shortcode( Almapay_WC_Cart_Handler $handler ) {
		add_shortcode(
	 ...
	 *
	 * @param Almapay_WC_Cart_Handler $handler Cart handler.
	 * @param array|string            $atts Shortcode attributes.
	 * @param string                  $content Shortcode content ([sc]content[/sc]).
	 *
	 * @return string the rendered cart eligibility widget to inject
	 */
	private function alma_cart_widget( Almapay_WC_Cart_Handler $handler, $atts, $content = '' ) {
	...
	private function render( $shortcode_tag, $atts, $alma_content, $shortcode_content = '' ) {
		...
		return sprintf( '<div class="%s %s" style="%s">%s<div class="Almapay_WC_content">%s</div>%s</div>', $shortcode_tag, $class, $style, $debug_content, do_shortcode( $shortcode_content ), $alma_content );
	 ...
	 *
	 * @param Almapay_WC_Product_Handler $handler as product widget handler.
	 *
	 * @return void
	 */
	public function init_product_widget_shortcode( Almapay_WC_Product_Handler $handler ) {
		add_shortcode(
	 ...
	 *
	 * @param Almapay_WC_Product_Handler $handler Cart handler.
	 * @param array|string               $atts Shortcode attributes.
	 * @param string                     $content Shortcode content ([sc]content[/sc]).
	 *
	 * @return string the rendered product eligibility widget to inject
	 */
	private function alma_product_widget( Almapay_WC_Product_Handler $handler, $atts, $content = '' ) {
