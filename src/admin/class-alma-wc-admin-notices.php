...
class Alma_WC_Admin_Notices {
	...
	public function admin_notices() {
		...
		foreach ( $this->notices as $notice_key => $notice ) {
			...
			if ( $notice['dismissible'] ) {
				?>
				<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'alma-gateway-for-woocommerce-hide-notice', $notice_key ), 'wc_alma_pay_hide_notices_nonce', Alma_WC_Helper_Constants::NOTICE_NONCE_NAME ) ); ?>"
	...
	public function hide_notices() {
		if ( isset( $_GET['alma-gateway-for-woocommerce-hide-notice'] ) && isset( $_GET[Alma_WC_Helper_Constants::NOTICE_NONCE_NAME] ) ) {
			if ( ! wp_verify_nonce( wc_clean( wp_unslash( $_GET[Alma_WC_Helper_Constants::NOTICE_NONCE_NAME] ) ), 'wc_alma_hide_notices_nonce' ) ) {
