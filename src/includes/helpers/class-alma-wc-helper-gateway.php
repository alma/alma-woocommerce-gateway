...
class Alma_WC_Helper_Gateway
{
	...
	public function __construct()
	{
		...
		$this->payment_helper = new Alma_WC_Helper_Payment();
	}
	...
	public function woocommerce_available_payment_gateways($available_gateways)
	{
		...
		$product_helper = new Alma_WC_Helper_Product();
	...
	public function woocommerce_gateway_title($title, $id)
	{
		...
		if (Alma_WC_Helper_Constants::GATEWAY_ID !== substr($id, 0, 4)) {
			return $title;
		}

		if (Alma_WC_Helper_Constants::GATEWAY_ID === $id) {
			$title = $this->alma_settings->get_title(Alma_WC_Helper_Constants::PAYMENT_METHOD_PNX);
		}

		if (Alma_WC_Helper_Constants::ALMA_GATEWAY_PAY_LATER === $id) {
			$title = $this->alma_settings->get_title(Alma_WC_Helper_Constants::PAYMENT_METHOD_PAY_LATER);
		}

		if (Alma_WC_Helper_Constants::ALMA_GATEWAY_PAY_MORE_THAN_FOUR === $id) {
			$title = $this->alma_settings->get_title(Alma_WC_Helper_Constants::PAYMENT_METHOD_PNX_PLUS_4);
		}
	...
	public function woocommerce_gateway_description($description, $id)
	{
		...
		if (Alma_WC_Helper_Constants::GATEWAY_ID === $id) {
			$description = $this->payment_helper->get_description(Alma_WC_Helper_Constants::PAYMENT_METHOD_PNX);
		}

		if (Alma_WC_Helper_Constants::ALMA_GATEWAY_PAY_LATER === $id) {
			$description = $this->payment_helper->get_description(Alma_WC_Helper_Constants::PAYMENT_METHOD_PAY_LATER);
		}

		if (Alma_WC_Helper_Constants::ALMA_GATEWAY_PAY_MORE_THAN_FOUR === $id) {
			$description = $this->payment_helper->get_description(Alma_WC_Helper_Constants::PAYMENT_METHOD_PNX_PLUS_4);
		}
	...
	public function get_default_plan($plans)
	{
		...
		if (in_array(Alma_WC_Helper_Constants::DEFAULT_FEE_PLAN, $plans, true)) {
			return Alma_WC_Helper_Constants::DEFAULT_FEE_PLAN;
		}
